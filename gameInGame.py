...
from normalGame import TicTacToeBoard as ng
...
class BigTicTacToeBoard(tk.Tk):
    ...
    def _create_big_board_grid(self):
        color = "#ECF0F1"
        for row in range(9):
    ...
    def set_big_title(self, newName):
        self.title(newName)
        self.reset_board(newName)
    
    def get_big_game_state(self, r, c):
        return self.game_state[r][c]
    
    def set_big_game_state(self, r, c, value):
        self.game_state[r][c] = value

    def check_for_big_win(self, player):
        for row in range(3):
    ...
    def check_for_big_tie(self):
        return all(self.game_state[row][col] != '' for row in range(3) for col in range(3))

    def display_big_message(self, message):
        messagebox.showinfo("Game Over", message)

    def reset_big_board(self, titleName = ""):
    ...
    def click_big_button(self, event, row, col):
        button = event.widget
